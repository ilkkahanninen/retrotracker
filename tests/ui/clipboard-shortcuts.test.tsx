...
describe("Cmd+C: copy", () => {
  ...
  it("with NO selection: copies the cursor's single cell", () => {
    setSong(songWith([{ row: 4, ch: 2, note: { period: C2, sample: 7 } }]));
    render(() => <App />);
...
describe("Cmd+E: bounce selection to sample", () => {
  ...
  it("is suppressed during playback", () => {
    setSong(songWith([{ row: 0, ch: 0, note: { period: C2, sample: 1 } }]));
    const s = song()!;
    s.samples[0] = {
      ...s.samples[0]!,
      lengthWords: 4,
...
describe("all clipboard shortcuts gate on transport", () => {
  it("Cmd+C / X / V are no-ops while playing", () => {
    ...
    setClipboardSlice({
      rows: [[{ period: D2, sample: 0, effect: 0, effectParam: 0 }]],
    });
    chord("v");
...
describe("cursor advances after paste", () => {
  it("paste lands the cursor on the row right after the pasted block", () => {
    ...
    render(() => <App />);
    setClipboardSlice({
      rows: [[{ period: C2, sample: 0, effect: 0, effectParam: 0 }]],
    });
