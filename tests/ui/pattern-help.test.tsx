...
describe("PatternHelp: context-sensitive tips", () => {
  it("on the note column, shows piano / edit step / sample / play / selection sections", () => {
    ...
    const { container } = render(() => <App />);
    fireEvent.click(
      container.querySelector<HTMLButtonElement>(".patternhelp__toggle")!,
    );
  ...
  it("on an effect column, shows the 16-effect grid plus the 16 extended sub-commands", () => {
    ...
    const mainCodes = Array.from(
      grids[0]!.querySelectorAll(".patternhelp__kbd"),
    ).map((el) => el.textContent);
    ...
    const extCodes = Array.from(
      grids[1]!.querySelectorAll(".patternhelp__kbd"),
    ).map((el) => el.textContent);
