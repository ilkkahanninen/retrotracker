...
describe("loadSession: missing / corrupt input", () => {
  ...
  it("returns null when the version field is missing or wrong", () => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ v: 99, songBase64: "" }),
    );
    expect(loadSession()).toBeNull();
  });
...
describe("App boot: restores from localStorage on mount", () => {
  ...
  it("uses the stored song / view / cursor instead of the empty default", async () => {
    ...
    const { song, setSong, setTransport, setPlayPos, clearHistory } =
      await import("../../src/state/song");
    const { setCursor, INITIAL_CURSOR, cursor } =
      await import("../../src/state/cursor");
    const {
      setCurrentSample,
      setCurrentOctave,
      setEditStep,
