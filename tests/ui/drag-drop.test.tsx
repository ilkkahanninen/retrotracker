...
import {
  setSong, song, setTransport, setPlayMode, setPlayPos, clearHistory, undo,
  setDirty,
} from '../../src/state/song';
import {
  setCurrentSample, currentSample, setCurrentOctave, setEditStep,
} from '../../src/state/edit';
import { setView } from '../../src/state/view';
import { clearAllWorkbenches, getWorkbench } from '../../src/state/sampleWorkbench';
...
describe('drag-drop: WAV imports fan out across free slots', () => {
  ...
  it('skips the current slot when it is already populated and uses the next free one', async () => {
    ...
    expect(Array.from(song()!.samples[0]!.data)).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
  ...
  it('emits a "skipped" error when there are more WAVs than free slots', async () => {
    ...
    await waitFor(() => {
      expect(song()!.samples[s.samples.length - 1]!.lengthWords).toBeGreaterThan(0);
    });
...
describe('drag-drop: routing by file extension', () => {
  ...
  it('an unsupported file (.txt) is ignored — no slot mutation, no workbench', async () => {
    ...
    setCurrentSample(1);
    const { container } = render(() => <App />);
    const txt = new File([new ArrayBuffer(0)], 'notes.txt', { type: 'text/plain' });
