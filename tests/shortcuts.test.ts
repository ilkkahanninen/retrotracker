...
describe("matchesShortcut", () => {
  ...
  it("matches Cmd+Z to Undo", () => {
    expect(matchesShortcut(ev({ key: "z", metaKey: true }), undoShortcut)).toBe(
      true,
    );
  });

  it("matches Ctrl+Z to Undo (cross-platform mod)", () => {
    expect(matchesShortcut(ev({ key: "z", ctrlKey: true }), undoShortcut)).toBe(
      true,
    );
  });
...
describe("position-based shortcuts (event.code matching)", () => {
  ...
  it("default-mode letter shortcut (Cmd+A) does NOT fire from QWERTY-A position on AZERTY", () => {
    ...
    expect(
      matchesShortcut(ev({ key: "q", code: "KeyA", metaKey: true }), cmdA),
    ).toBe(false);
    // It does still fire when the user types A on any layout.
    expect(
      matchesShortcut(ev({ key: "a", code: "KeyQ", metaKey: true }), cmdA),
    ).toBe(true);
  });
});
  ...
  function makeFakeWindow() {
    ...
    return {
      ...
      keydown: (init: Partial<KeyboardEvent>) =>
        (listeners.keydown ?? []).forEach((l) =>
          l(ev(init) as unknown as Event),
        ),
...
describe("Space chord shortcuts", () => {
  it("matchesShortcut routes Space + modifier permutations correctly", () => {
    ...
    expect(
      matchesShortcut(ev({ key: " ", altKey: true, shiftKey: true }), alt),
    ).toBe(false);

    expect(matchesShortcut(ev({ key: " ", shiftKey: true }), shift)).toBe(true);
    expect(
      matchesShortcut(ev({ key: " ", shiftKey: true, altKey: true }), shift),
    ).toBe(false);

    expect(
      matchesShortcut(ev({ key: " ", altKey: true, shiftKey: true }), altShift),
    ).toBe(true);
  });
  ...
  it("matches by event.code when macOS munges Alt+Space to NBSP", () => {
    ...
    // code stays 'Space'.
    expect(
      matchesShortcut(ev({ key: " ", code: "Space", altKey: true }), alt),
    ).toBe(true);
  });
  ...
  it("matches Shift + comma/period via event.code on US layouts", () => {
    ...
    expect(
      matchesShortcut(ev({ key: "<", code: "Comma", shiftKey: true }), prevPat),
    ).toBe(true);
    expect(
      matchesShortcut(
        ev({ key: ">", code: "Period", shiftKey: true }),
        nextPat,
      ),
    ).toBe(true);
    // Plain ',' / '.' (no shift) doesn't accidentally satisfy the shifted shortcut.
    expect(matchesShortcut(ev({ key: ",", code: "Comma" }), prevPat)).toBe(
      false,
    );
  });
});
