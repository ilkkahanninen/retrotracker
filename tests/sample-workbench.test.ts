...
function stereo(L: number[], R: number[]) {
  return {
    sampleRate: sr,
    channels: [Float32Array.from(L), Float32Array.from(R)],
  };
}
...
describe("applyFilter", () => {
  ...
  it("low-pass at high cutoff barely attenuates a low-frequency sine", () => {
    ...
    for (let i = 0; i < N; i++)
      sig[i] = Math.sin((2 * Math.PI * 1000 * i) / sr);
  ...
  it("low-pass at low cutoff strongly attenuates a high-frequency sine", () => {
    ...
    const sig = new Float32Array(N);
    for (let i = 0; i < N; i++)
      sig[i] = Math.sin((2 * Math.PI * 8000 * i) / sr);
    const out = applyFilter(
      { sampleRate: sr, channels: [sig] },
      "lowpass",
      500,
...
describe("applyCrossfade", () => {
  ...
  it("is a no-op when there is no pre-loop tail", () => {
    ...
    const out = applyCrossfade({ sampleRate: 44100, channels: [ch] }, 2, 0, 4);
...
describe("transformToPt (mono mix + int8 quantise)", () => {
  it("passes through a mono source", () => {
    const out = transformToPt(mono(0, 1, -1), {
      monoMix: "average",
      targetNote: null,
    });
  ...
  it("mono mix = average sums and divides stereo", () => {
    const out = transformToPt(stereo([1, 0], [-1, 0]), {
      monoMix: "average",
      targetNote: null,
    });
  ...
  it("mono mix = left picks just the left channel", () => {
    const out = transformToPt(stereo([1, 0.5], [-1, -0.5]), {
      monoMix: "left",
      targetNote: null,
    });
...
describe("transformToPt dither", () => {
  it("omitting dither (or setting it false) is bit-identical to the un-dithered quantiser", () => {
    const audio = {
      sampleRate: 8287,
      channels: [new Float32Array([0, 0.5, -0.5, 0.25, -0.25, 1, -1])],
    };
    const a = transformToPt(audio, { monoMix: "average", targetNote: null });
    const b = transformToPt(audio, {
      monoMix: "average",
      targetNote: null,
      dither: false,
    });
  ...
  it("decorrelates the quantisation error: low-level signal produces non-zero output (vs. all-zero un-dithered)", () => {
    ...
    const audio = { sampleRate: 8287, channels: [ch] };
    const undith = transformToPt(audio, {
      monoMix: "average",
      targetNote: null,
      dither: false,
    });
    const dith = transformToPt(audio, {
      monoMix: "average",
      targetNote: null,
      dither: true,
    });
...
describe("SampleSource", () => {
  ...
  it("materializeSource returns the WAV for sampler and a synth cycle for chiptune", () => {
    ...
    const chip = workbenchFromChiptune({
  ...
  it('sourceDisplayName falls back to "Chiptune" for the synth source', () => {
    expect(
      sourceDisplayName({ kind: "chiptune", params: defaultChiptuneParams() }),
    ).toBe("Chiptune");
    expect(
      sourceDisplayName({
        kind: "sampler",
...
describe("resampleFilteredLinear", () => {
  ...
  it("attenuates an above-Nyquist sine far more than plain linear", () => {
    ...
    const sig = new Float32Array(N);
    for (let i = 0; i < N; i++)
      sig[i] = Math.sin((2 * Math.PI * 6000 * i) / fromRate);
...
describe("transformToPt resampleMode dispatch", () => {
  ...
  it("'sinc' produces a different waveform from 'linear' on a downsample", () => {
    ...
    const linear = transformToPt(audio, {
      monoMix: "average",
      targetNote: 12,
      resampleMode: "linear",
    });
    const sinc = transformToPt(audio, {
      monoMix: "average",
      targetNote: 12,
      resampleMode: "sinc",
    });
...
describe("transformToPt with targetNote", () => {
  it("null targetNote leaves the source rate alone (no resample)", () => {
    const audio = {
      sampleRate: 44100,
      channels: [new Float32Array([0, 1, -1])],
    };
  ...
  it("dispatches each kind to its specialised function", () => {
    ...
    expect(
      applyEffect(mono(1, 2, 3), {
        kind: "reverse",
        params: { startFrame: 0, endFrame: 3 },
      }).channels[0],
    ).toEqual(Float32Array.from([3, 2, 1]));
    expect(
      applyEffect(mono(0, 1, 2, 3, 4), {
        kind: "cut",
        params: { startFrame: 1, endFrame: 4 },
      }).channels[0],
    ).toEqual(Float32Array.from([0, 4]));
  });
