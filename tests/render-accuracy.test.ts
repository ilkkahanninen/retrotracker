...
function ensureRenderBinary(): void {
  ...
  console.log('[accuracy] vendor/bin/pt2-render not built — running build script (one-time, ~1s)');
...
function ensureReferenceWav(fx: FixtureCase): void {
  ...
  execFileSync(RENDER_BIN, [fx.modPath, fx.wavPath, '--rate=44100'], { stdio: 'inherit' });
}
