...
import {
  deriveSampleName, importWavSample, wavToInt8Mono,
...
describe('deriveSampleName', () => {
  ...
  it('truncates to the 22-char PT name limit', () => {
    expect(deriveSampleName('a-very-long-filename-indeed.wav'))
      .toBe('a-very-long-filename-i');
  });
...
describe('importWavSample (round-trip from a synthetic WAV)', () => {
  it('parses a 16-bit mono WAV into the expected int8 mono data', () => {
    ...
    const wav = writeWav({ sampleRate: 22050, channels }, { bitsPerSample: 16 });
  ...
  it('mixes a stereo WAV down to mono', () => {
    ...
    const wav = writeWav({ sampleRate: 44100, channels }, { bitsPerSample: 16 });
