...
import {
  selection, setSelection, clearSelection, makeSelection,
  selectionContains, selectionSize,
...
describe('selection state', () => {
  ...
  it('makeSelection normalises swapped row / channel endpoints', () => {
    ...
    expect(sel).toEqual({
      order: 0, startRow: 1, endRow: 5, startChannel: 0, endChannel: 3,
    });
  });
  ...
  it('selectionSize counts inclusive cells on both axes', () => {
    expect(selectionSize(makeSelection(0, 0, 0, 0, 0))).toEqual({ rows: 1, channels: 1 });
    expect(selectionSize(makeSelection(0, 1, 0, 4, 2))).toEqual({ rows: 4, channels: 3 });
  });
